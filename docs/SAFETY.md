...
全部 86 条规则集中在 [`RuleCatalog.swift`](../Sources/MacVitalKit/Rules/RuleCatalog.swift) 一个文件。
...
| 5c | 内容能否全部移除 | ACL 禁止删除、或非空目录不可写。**属主是别人（root）时另算**：那是权限问题不是永久问题，root 能做到，所以走特权助手而不是拒绝 |
...
测试跑 `make test`：247 个核心及网卡路由测试，另有 3 个不启动应用的绘图业务测试。

还原和清除必须先持久化操作标记，标记写入失败时恢复内存状态，保持记录可见且可重试；到期清扫每项执行前重新检查正在进行的操作，避免等待助手期间与还原重叠。
