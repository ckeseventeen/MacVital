...
macOS 空间清理、截图与屏幕画笔演示利器。核心不是「能删」，而是**敢删且不出事**，同时提供丝滑的屏幕截图、画笔批注与白板演示工作流。

清理类工具的难点从来不是找到文件，而是证明这个文件可以删。PureMark 把「能不能删」做成了一条确定性的、可审计的、默认拒绝的规则链——模型只负责解释，用户只负责授权，三者取交集才执行，而且执行本身也不是删除，是移进可还原的隔离区。
...
- **[docs/AUDIT-2026-09-12.md](docs/AUDIT-2026-09-12.md)** —— 产品缺口、已修复问题、业务测试矩阵与架构改进顺序。
...
全部 86 条规则在 [RuleCatalog.swift](Sources/MacVitalKit/Rules/RuleCatalog.swift) 一个文件里。不在这份白名单里的路径，任何扫描器都提不出来，任何用户手势也选不中。
...
`allowedInUserData` 只有 16 条规则打开：6 条构建产物（node_modules / Pods / .build / target / .venv / .next），以及大文件和重复文件在 5 个用户目录（下载 / 文稿 / 桌面 / 影片 / 图片）下各自的一条。有测试锁死这份清单。特权规则 8 条，全部位于测试显式列出的系统根下。
...
Tests/MacVitalKitTests/ 247 个核心及网卡路由测试
Tests/MacVitalAppLogicTests/ 3 个绘图业务测试（不启动应用）
